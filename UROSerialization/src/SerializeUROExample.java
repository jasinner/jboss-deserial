...
public class SerializeUROExample {
    ...
    public static Object createObject(int port) throws Exception {
        Constructor objCons = RemoteObject.class.getDeclaredConstructor(new Class[] { RemoteRef.class });
        Constructor serializationConstructor = ReflectionFactory.getReflectionFactory()
                .newConstructorForSerialization(ActivationGroupImpl.class, objCons);
        serializationConstructor.setAccessible(true);
        return serializationConstructor.newInstance(new Object[] { new UnicastServerRef(port) });
    }
}
